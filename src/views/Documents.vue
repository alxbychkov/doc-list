...
const DOCUMENT_JSON = {
    CATEGORIES: [
        ...
        },
        {
            ...
            types: [],
            description: 'Документы, без которых невозможно трудоустройство человека на какую бы то ни было должность в компании вне ависимости от граж',
            list: []
        },
        {
            id: '3',
...
const filteredCategories = computed(() => categories.value.filter(c => ~c.title.toLowerCase().indexOf(search.value.toLowerCase())));
const filteredList = computed(() => list.value.filter(c => ~c.title.toLowerCase().indexOf(search.value.toLowerCase())));
        ...
        <div class="main-body">
            <ul class="category-list list">
                <li v-for="category in filteredCategories" :key="category.id">
                    <div class="category-list-item" :class="category.list.length || 'empty'" @click="listItemClickHandler">
                        <span class="category-list-title">{{ category.title }}</span>
                        <span v-if="category.types.length" class="list-color">
                            <i v-for="t in category.types" class="circle" :class="t" :key="t"></i>
                        </span>
                        <span v-if="category.description" class="list-description">{{ category.description }}</span>
                        <span class="list-action">
                            <i class="action-edit"></i>
                            <i class="action-delete"></i>
                            <i class="action-move"></i>
                        </span>
                    </div>
                        ...
                        <ul class="list">
                            <li v-for="l in category.list" :key="l.id">
                                <div class="list-item">
                                    ...
                                    <span v-if="l.format" class="list-description pink-color">{{ l.format }}</span>
                                    <span v-if="l.description" class="list-description">{{ l.description }}</span>
                                    <span class="list-action">
                                        <i class="action-edit"></i>
                                        <i class="action-delete"></i>
                                        <i class="action-move"></i>
